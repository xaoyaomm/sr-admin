 ...
 * 框架会默认调用DWZ.ajaxDone() <form action="/user.do?method=save"
...
public class AjaxObject {
	...
	public static AjaxObject newForward(String forwardUrl) {
		...
		ajaxObject.setCallbackType(CALLBACK_TYPE_FORWARD);
		ajaxObject.setStatusCode(STATUS_CODE_SUCCESS);
		ajaxObject.forwardUrl = forwardUrl;
		return ajaxObject;
	}

	public static AjaxObject newForward(String forwardUrl, String message,
			int statusCode) {
		AjaxObject ajaxObject = new AjaxObject();
		ajaxObject.setCallbackType(CALLBACK_TYPE_FORWARD);
		ajaxObject.setStatusCode(statusCode);
		ajaxObject.forwardUrl = forwardUrl;
