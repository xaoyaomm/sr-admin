...
function validateCallbackUrl(url,paramter, callback, confirmMsg) {
	var _submitFn = function(){
		$.ajax({
			type:  'POST',
			url:url,
...
function navTabAjaxDone(json){
	...
	if (json.statusCode == DWZ.statusCode.ok){
		...
		if(json.rel && json.rel == "order_info"){
			navTab.closeCurrentTab("order_info");
		}
...
function divUrlSearch(ur,paramter, rel){
 
	if (rel) {
		var $box = $("#" + rel);
		$box.ajaxUrl({
			type:"POST", url:ur, data:paramter, callback:function(){
				$box.find("[layoutH]").layoutH();
			}
		});
	}
...
function divUrlSearchRelObj(ur,paramter, rel,relInitMethod){
	if (rel) {
		var $box = $(rel);
		$box.ajaxUrl({
			type:"POST", url:ur, data:paramter, callback:function(){
